...
public class ClickFCAccount extends AnkiActivity {
    private final static int STATE_LOG_IN = 1;
    private final static int STATE_LOGGED_IN = 2;

    private View mLoginToClickFCAccountView;
    private View mLoggedIntoClickFCAccountView;
    ...
    private void switchToState(int newState) {
        switch (newState) {
            case STATE_LOGGED_IN:
                String username = ClickFCApp.getSharedPrefs(getBaseContext()).getString("username", "");
                mUsernameLoggedIn.setText(username);
                mToolbar = (Toolbar) mLoggedIntoClickFCAccountView.findViewById(R.id.toolbar);
                if (mToolbar != null) {
                ...
                setContentView(mLoggedIntoClickFCAccountView);
                break;

            case STATE_LOG_IN:
                mToolbar = (Toolbar) mLoginToClickFCAccountView.findViewById(R.id.toolbar);
                if (mToolbar != null) {
                    mToolbar.setTitle(getString(R.string.ninja_sync_account));  // This can be cleaned up if all three main layouts are guaranteed to share the same toolbar object
                    setSupportActionBar(mToolbar);
                }
                setContentView(mLoginToClickFCAccountView);
                break;
        }
    ...
    private void initAllContentViews() {
        mLoginToClickFCAccountView = getLayoutInflater().inflate(R.layout.clickfc_account, null);
        mUsername = (EditText) mLoginToClickFCAccountView.findViewById(R.id.username);
        mPassword = (EditText) mLoginToClickFCAccountView.findViewById(R.id.password);

        Button loginButton = (Button) mLoginToClickFCAccountView.findViewById(R.id.login_button);
        ...
        Button resetPWButton = (Button) mLoginToClickFCAccountView.findViewById(R.id.reset_ninja_password_button);
        ...
        mLoggedIntoClickFCAccountView = getLayoutInflater().inflate(R.layout.click_fc_account_logged_in, null);
        mUsernameLoggedIn = (TextView) mLoggedIntoClickFCAccountView.findViewById(R.id.username_logged_in);
        Button logoutButton = (Button) mLoggedIntoClickFCAccountView.findViewById(R.id.logout_button);
    ...
    Connection.TaskListener loginListener = new Connection.TaskListener() {
        ...
        public void onPreExecute() {
            ...
            if (mProgressDialog == null || !mProgressDialog.isShowing()) {
                mProgressDialog = StyledProgressDialog.show(ClickFCAccount.this, "",
        ...
        public void onPostExecute(Payload data) {
            ...
            if (data.success) {
                ...
                saveClickFCUserInformation((String) data.data[0], (String) data.data[1],(List<Produto>)data.result);

                Intent i = ClickFCAccount.this.getIntent();
                if (i.hasExtra("notLoggedIn") && i.getExtras().getBoolean("notLoggedIn", false)) {
                    ClickFCAccount.this.setResult(RESULT_OK, i);
                ...
                if (data.returnType == 401) {
                    ...
                    mayOpenUrl(helpUrl);
                    UIUtils.showSnackbar(ClickFCAccount.this, R.string.wrong_ninja_password, false, R.string.wrong_ninja_help, new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            openUrl(helpUrl);
                        }
                    ...
                    UIUtils.showSimpleSnackbar(ClickFCAccount.this, R.string.invalid_ninja_username_password, true);
                } else {
                    UIUtils.showSimpleSnackbar(ClickFCAccount.this, R.string.connection_error_message, true);
                }
            }
        }
        ...
        public void onDisconnected() {
            UIUtils.showSimpleSnackbar(ClickFCAccount.this, R.string.youre_offline, true);
        }
    };
