...
public class Info extends AnkiActivity {
            ...
            public void onClick(View arg0) {
                if (mType == TYPE_ABOUT) {
                    if (CompatHelper.isKindle()) {
                        Intent intent = new Intent("android.intent.action.VIEW",
                                Uri.parse("http://www.amazon.com/gp/mas/dl/android?p=com.ichi2.clickfc"));
                        startActivity(intent);
                    } else {
                        Info.this.startActivity(new Intent(Intent.ACTION_VIEW, Uri
                                .parse("market://details?id=com.ichi2.clickfc")));
                    }
                    return;
                }
                setResult(RESULT_OK);
                switch (mType) {
                    case TYPE_NEW_VERSION:
                        ClickFCApp.getSharedPrefs(Info.this.getBaseContext()).edit()
    ...
    public String copyDebugInfo() {
        StringBuilder sb = new StringBuilder();
        // AnkiDroid Version
        sb.append("ClickFC Version = ").append(VersionUtils.getPkgVersionName()).append("\n\n");
        // Android SDK
        sb.append("ClickFC Version = " + Build.VERSION.RELEASE).append("\n\n");
        // ACRA install ID
